...
impl Sock {
    pub fn new(name: &str, address: SocketAddr, rate: f64, targets: Vec<String>) -> Sock {
        Sock {
            address: address,
            send_count: 0,
            recv_count: 0,
...
impl Sockage {
    pub fn default(name: &str, addr: SocketAddr, rate: u128) -> Sockage {
        Sockage {
            ...
            node: Sock::new(name, address, rate, vec![]),
    ...
    pub fn clean_registry(&mut self) {
        self.socks.retain(|sock| sock.timestamp <= 5.0 / sock.rate);
    }
