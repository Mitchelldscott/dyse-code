...
def parse_args(args):
	"""
		...
		ros-nodes:
		  ...
		  files: [rqt_plot]
		  package: rqt_plot
		  args: ['-c', '${PROJECT_ROOT}/dysepy/data/robots/penguin/default.perspective']
	"""
	clean_args = []
	for arg in args:
		# find any env vars
...
def get_devices():
	"""
		Load all registered devices from dysepy/data/robots/robots.yaml
	"""
	robots = os.path.join(DysePy_LOC_LUT['robots'], 'robots.yaml')
