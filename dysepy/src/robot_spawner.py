...
class Robot_Spawner:
	...
	def spin(self, robot):
		...
		try: 
			...
			while 1: #self.roscore.poll() is None:
...
def main():
	...
	"""
	if len(sys.argv) < 2:
		with open(os.path.join(DysePy_LOC_LUT['robots'], 'self.txt'), 'r') as f:
			robot = f.read()
	else:
		robot = sys.argv[1]
		# with open(os.path.join(DysePy_LOC_LUT['robots'], 'self.txt'), 'w') as f:
		# 	f.write(robot)
