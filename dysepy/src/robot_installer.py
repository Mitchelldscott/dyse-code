...
def deploy_all_devices():
	...
	for robot in devices:
		...
		try:	
			clean_dir_result = sb.run(['ssh', robot_address, f'if [[ -d {target}/dysepy ]]; then rm -rf {target}/dysepy; fi; mkdir -p {target}'], timeout=30)
			if clean_dir_result.returncode == 0:
...
def initialize_devices():
	"""
		Deploy dysepy and run installation on
	...
	data_target = os.path.join(target, 'dysepy', 'data')
	src_target = os.path.join(target, 'dysepy')
	...
	for robot in devices:
		...
		# deploy dysepy and source the install scripts
		...
		try:
			setup_result = sb.run(['ssh', 
					'-t',
					robot_address, 
					f'cd /home/dyse-robotics/dyse-code && source dysepy/scripts/install.bash'])
