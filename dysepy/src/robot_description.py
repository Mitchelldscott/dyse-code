...
class Robot_Description:
	...
	def dyse_node_commands(self, nodes):
		...
		for name in nodes:
			...
			if 'files' in node:
				for program in node['files']:
					file_path = os.path.join(DysePy_LOC_LUT['lib'], program)
					if '.py' == program[-3:]:
	...
	def load_description(self, name):
		...
		self.data = {}
		self.name = name
		self.path = os.path.join(DysePy_LOC_LUT['robots'], self.name, 'nodes.yaml')
		...
		proc_data = os.path.join(DysePy_LOC_LUT['robots'], name, 'nodes.yaml')
